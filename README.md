...
| Frontend      | Angular 21 (signals, standalone)    | SSE client, no polling |
...
- **Rich observability** — Per-machine status pills, metrics, and quality bars. Header tally counts machines by state.
...
└── web/                    # Angular 21 SPA (served from API wwwroot in prod)
    └── src/app/factory/    # Production line, parts panel, command drawer, SSE service
```
